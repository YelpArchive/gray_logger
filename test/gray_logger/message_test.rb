...
class MessageTest < Test::Unit::TestCase
  ...
  def test_prefixes_non_reserved_keys_with_underscores
    ...
    assert message.to_hash[:_my_custom_field] == 'my custom message', "_my_custom_field isn't set: #{message.to_hash.inspect}"
    assert message.to_hash[:my_custom_field].nil?, "my_custom_field is set: #{message.to_hash.inspect}"
  end

  def test_does_not_prefix_reserved_keys_with_underscores
    message = GrayLogger::Message.new({:level => 1})
    assert message.to_hash[:level] == 1, "level isn't set: #{message.to_hash.inspect}"
